      ...
      {idx === 2 ? (
        ...
        <>
          {data.length - 1 === idx ? (
            <p
              key={idx + task.task}
              style={{ color: task.isChecked === true ? "green" : "red" }}
            >
              {task.task}
            </p>
          ) : (
      ...
      <tr>
        {toDelete === false ? (
          <>
            ...
            <td>
              {data.oque.length > 35
                ? data.oque.substr(0, 35) + "..."
                : data.oque}
            </td>
            <td>
              {data.prioridade === "Pode esperar" ? (
                <p style={{ color: "green", fontWeight: "bold" }}>
                  {data.prioridade}
                </p>
              ) : data.prioridade === "Pouco urgente" ? (
            ...
            <td>{getStatus()}</td>
            <td>{getConclusao() * 100}%</td>
            <td>{getDiff()}%</td>
            <td>
              {data.isReajustado === true ? (
                <div className="reajustado-positive-indicator" />
              ) : (
                <div className="reajustado-negative-indicator" />
              )}
            </td>
          </>
        ) : (
          <>
            ...
            <p className="confirm-message-table">
              Você tem certeza de que deseja apagar essa meta?
            </p>
          </>
        )}
