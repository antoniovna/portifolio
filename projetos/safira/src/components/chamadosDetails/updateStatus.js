...
export default function UpdateStatus(props) {
    ...
    const submit = () => {
        ...
        if (newStatus === "Feito") {
            if (props.operation === 1) {
                const ref = firebase.firestore().collection("Chamados").doc(props.data.id).update({
                    finalized: true,
                    status: "Feito"
                }).then(() => {
                    if (note != "") {
                ...
                firebase.firestore().collection("Chamados").doc(props.data.id).update({
                    status: "Feito",
    ...
    const makeNotes = async (id) => {
        var name;
        await firebase.firestore().collection("Users").where('email', "==", firebase.auth().currentUser.email).get().then(userData => {
        ...
        firebase.firestore().collection("Notes").add({
            ...
            firebase.firestore().collection("Notes").doc(newDOc.id).update({ id: newDOc.id }).then(() => {
                setNewStatus("");
                setNewStatus("");
                handleClick();
            })
    ...
    const makeNotification = async (id, status, operation) => {
        ...
        if (operation === 1) {///notificação deve ir para os encarregados!
        ...
        } else { //chamado deve ir para o dono
            firebase.firestore().collection("Notifications").add({
                relatedId: '',
                userName: name,
    ...
    return (
        <>
            ...
            <Snackbar open={isEmpty} autoHideDuration={6000} onClose={handleClickError}>
                        ...
                        <CardBody>
                            <Form>
                                <Col className="pr-md-1" md="12">
                                    ...
                                    <Input value={newStatus} type="select" placeholder="Anotação" onChange={(e) => setNewStatus(e.target.value)} >
                                        {
